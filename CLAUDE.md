...
Local-only Path of Exile tool inspired by [poe-disenchant-tool](https://github.com/deronek/poe-disenchant-tool). The reference app is a Next.js web app deployed on Vercel that helps players find the most cost-efficient unique items to disenchant for Thaumaturgic Dust. Our project takes a different approach: **local-only execution**, no cloud deployment.

The reference code lives in `/reference/` for study purposes.

## Reference Architecture (poe-disenchant-tool)

### How It Works

1. **Fetches real-time prices** from poe.ninja (public API, no auth)
2. **Loads static dust values** from a pre-generated JS file (processed from CSV)
3. **Merges data**: matches market prices to dust values per unique item
4. **Calculates efficiency**: `dustPerChaos = calculatedDustValue / chaosPrice`
5. **Renders sortable/filterable table** so players pick the best items to buy and disenchant
6. **Generates trade links** pointing to pathofexile.com/trade with pre-filled search params

### Data Flow

```
poe.ninja (prices) ─┐
                     ├─→ merge & calculate efficiency ─→ sortable table ─→ trade links
static dust data ────┘
```

### Key Calculations

- **Dust value**: `baseDust * 125 * (20 - (84 - clampedIlvl)) * ((quality * 2 + 100) / 100)`
- **Gold cost**: `floor(2000 * round(baseDust ^ 0.45, 2) * (1 + 0.02*quality + 0.5*(influence + corruption)))`
- **Efficiency**: `dustPerChaos = calculatedDustValue / chaosPrice` (primary sort metric)
- **Catalyst logic** (accessories): compares `dustQ20 / (price + catalystCost)` vs `dustQ0 / price` to recommend whether to use catalysts

## External APIs & Data Sources

### 1. poe.ninja API (Public, No Auth)

**Item prices** — `GET https://poe.ninja/api/data/itemoverview`
- Params: `type` (`UniqueWeapon`|`UniqueArmour`|`UniqueAccessory`), `league`
- Returns: `{ lines: [{ name, chaosValue, divineValue, baseType, icon, listingCount, detailsId, itemType }] }`
- Rate limit: Informal, use a descriptive User-Agent header
- Caching recommended: 30 minutes

**Currency rates** — `GET https://poe.ninja/poe1/api/economy/exchange/current/overview`
- Params: `league`, `type=Currency`
- Returns: `{ lines: [{ id, primaryValue }], core: { rates: { divine } } }`
- Used for: cheapest catalyst price, divine-to-chaos exchange rate
- Caching recommended: 1 day

### 2. Path of Exile Official Trade (Link Generation Only)

**No API calls made** — the reference app only generates URLs with query params:
- `https://www.pathofexile.com/trade/search/{league}?q={encodedJSON}`
- The JSON payload specifies: item name, online status, min ilvl, corruption filter, listing time
- Users click the link and browse trade results in their own browser

### 3. Static Dust Data

- Pre-generated from a CSV via `scripts/process-dust-data.ts`
- Contains per-item: `name`, `baseType`, `dustValIlvl84`, `dustValIlvl84Q20`, `goldCost`, `slots`
- This data changes only when GGG patches dust values (rare, league-specific)

### API Alternatives for Local Tool

| Need | Reference Uses | Alternatives |
|------|---------------|-------------|
| Item prices | poe.ninja (scrapes trade data) | poe.ninja (same, it's public), or PoE official trade API (requires OAuth) |
| Currency rates | poe.ninja exchange endpoint | Same as above |
| Dust values | Static CSV/JS file | Same approach — maintain a local data file |
| Trade search | URL generation (no API call) | Same approach, or use PoE Trade API directly (OAuth required) |
| Item metadata | poe.ninja response fields | PoE official API (`pathofexile.com/api`), poedb.tw, RePoE (GitHub data files) |

**PoE Official Trade API** (if we want direct trade integration):
- Requires OAuth2 via pathofexile.com
- Rate limited (more strictly than poe.ninja)
- Endpoint: `https://www.pathofexile.com/api/trade/search/{league}` (POST)
- Fetch results: `https://www.pathofexile.com/api/trade/fetch/{ids}` (GET)

## Reference Project Structure (Key Files)

| File | Role |
|------|------|
| `src/lib/prices/prices.ts` | poe.ninja fetch + variant deduplication |
| `src/lib/prices/currency.ts` | Currency/catalyst rates |
| `src/lib/dust/calculation.ts` | Dust value formula |
| `src/lib/dust/dust.ts` | Dust data loader |
| `src/lib/gold/calculation.ts` | Async trade gold fee formula |
| `src/lib/itemData/itemData.ts` | Merges price + dust → final Item type |
| `src/lib/tradeLink.ts` | Trade URL generation |
| `src/lib/leagues.ts` | League definitions |
| `src/lib/itemData/ignore-list.ts` | Non-disenchantable items |
| `scripts/process-dust-data.ts` | CSV → JS dust data generator |

## Reference Tech Stack

- Next.js 16 (App Router, Server Components), React 19, TypeScript 5
- TanStack Table v8, Tailwind CSS 4, shadcn/ui, Zod v4
- Vitest (unit), Playwright (E2E + visual regression)
...
pnpm test:desktop         # Playwright E2E (desktop)
pnpm test:mobile          # Playwright E2E (mobile)
pnpm lint                 # prettier + eslint + tsc
pnpm dust:process         # Re-process dust CSV → JS
```

## Leagues (Current)

```
standard, hardcore, mirage, hardcore-mirage
```

Default: `mirage` (current challenge league). League names must match poe.ninja's expected values.

## Important Domain Concepts

- **Thaumaturgic Dust**: Currency obtained by disenchanting (vendoring) unique items at Kingsmarch NPCs
- **Catalyst**: Quality currency for accessories; 20 catalysts = 20% quality = +40% dust
- **Variant deduplication**: poe.ninja lists items with suffixes like `-relic`, `-5l`, `-6l`; the tool groups these and uses the cheapest non-special variant
- **Foulborn**: Special variant prefix (e.g., "Foulborn Kaom's Heart") that gets merged with its non-Foulborn counterpart
- **Low stock threshold**: 10th percentile of listing counts — items below this show a warning
- **Item level 65-84**: Affects dust multiplier; default filter is ilvl 78+
...
- **One logical commit per task** is the default. Split into multiple commits only when the task has clearly separable pieces (e.g., refactor + feature).
- **Never leave uncommitted work between tasks.** If switching context, commit or stash first.
